...
unique_x, unique_indices = torch.unique(data.x, dim=0, return_inverse=True)

keep_mask = torch.zeros(data.num_nodes, dtype=torch.bool)
keep_mask[torch.unique(unique_indices, sorted=True)] = True

old_to_new = -torch.ones(data.num_nodes, dtype=torch.long)
old_to_new[keep_mask] = torch.arange(keep_mask.sum())

row, col = data.edge_index
mask_edges = keep_mask[row] & keep_mask[col]

row = old_to_new[row[mask_edges]]
col = old_to_new[col[mask_edges]]

data.edge_index = torch.stack([row, col], dim=0)
data.x = data.x[keep_mask]
data.y = data.y[keep_mask]

data.train_mask = data.train_mask[keep_mask]
data.val_mask = data.val_mask[keep_mask]
data.test_mask = data.test_mask[keep_mask]
